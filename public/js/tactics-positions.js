/*
  Tactics Positions (A/B lineups)
  - Uses Attendance.position as an encoded value: "A:GK", "B:ST2" etc.
  - Loads from GET /activities/:id
  - Saves via PATCH /activities/:id/positions (admin only)

  NOTE: This is intentionally simple so we can iterate fast.
*/
...
(function(){
  ...
  const benchAside = document.getElementById('benchAside');
  const benchList = document.getElementById('benchList');
  const benchEmpty = document.getElementById('benchEmpty');
  const benchTitle = document.getElementById('benchTitle');

  const benchListMobile = document.getElementById('benchListMobile');
  const benchEmptyMobile = document.getElementById('benchEmptyMobile');
  const benchTitleMobile = document.getElementById('benchTitleMobile');

  const teamSeg = document.getElementById('teamSeg');
  const formationSeg = document.getElementById('formationSeg');

  const btnOpenBench = document.getElementById('btnOpenBench');
  const btnCloseBench = document.getElementById('btnCloseBench');
  const benchDrawer = document.getElementById('benchDrawer');

  const backLink = document.getElementById('backLink');
  if (backLink && activityId) backLink.href = '/tactics.html?activityId=' + encodeURIComponent(activityId);
  ...
  function findDrop(x,y){
    ...
    if (!el) return null;
    return el.closest('[data-slot-key], #benchList, #benchListMobile');
  }
  ...
  function finishDrop(x,y,cancelled){
    ...
    if (!cancelled && x!=null && y!=null) {
      ...
      if (drop) {
        ...
        if (uid) {
          if (drop.id === 'benchList' || drop.id === 'benchListMobile') {
            unassignUser(uid);
          } else {
  ...
  function assignToSlot(userId, team, slotKey){
    ...
    // If user already assigned elsewhere, clear it
    if (cur) {
      delete state.lineup[cur.team][cur.slotKey];
    }

    // If slot occupied, move that user back to bench (or to user's old slot?)
  ...
  async function save(opts){
    ...
    // Build positions for both teams
    const positions = [];
  ...
  function renderBench(){
    ...
    // bench = registered users not assigned in activeTeam and belonging to active team by teamNo
    const bench = [];
    ...
    function fill(container, emptyEl, titleEl){
      if (!container) return;
      container.innerHTML = '';
      if (titleEl) titleEl.textContent = `候补（${bench.length}）`;
      if (!bench.length) {
    ...
    fill(benchList, benchEmpty, benchTitle);
    fill(benchListMobile, benchEmptyMobile, benchTitleMobile);
  }
  ...
  async function load(){
    ...
    // reset lineup + assigned
    state.lineup = { A: {}, B: {} };
    state.assigned = {};

    // Parse existing encoded positions
    atts.forEach((a)=>{
  ...
  function openBench(){
    if (!benchDrawer) return;
    benchDrawer.classList.remove('hidden');
  }
  function closeBench(){
    if (!benchDrawer) return;
    benchDrawer.classList.add('hidden');
  }
  if (btnOpenBench) btnOpenBench.addEventListener('click', openBench);
  if (btnCloseBench) btnCloseBench.addEventListener('click', closeBench);
